...
"""
input my_legal_nli_data into OCNLI_classifier, keep the highest probability one per sentence.

use_one_sentence_list as a batch ,size=21, choose the highest one leave

how to use NLI_model to classify, how to write the code?  maybe use the evaluation/prediction code directly

"""

""" Finetuning the library models for sequence classification on CLUE (Bert, ERNIE, XLNet, RoBERTa)."""

"""--------------如果是要预测----------------前6个参数需要随时改--test.json要随时换  //best_OCNLI_classify
--model_type=bert 
--model_name_or_path="./outputs/best_OCNLI_classify/"
--output_filter_dir_name="./my_data/ogfilter_output/chu_test_neutral.json"
--output_dir_add="chu_test_neutral/"
--now_right_label=2
--output_dir="./my_data/ogfilter_output/"
--task_name="ocnli"
--do_predict 
--do_lower_case 
--data_dir="./my_data/"
...
--seed=42
--------------------------------------"""
"""---------------------------前6个参数需要随时改----  //best_OCNLI_classify
python filter_sentence.py \
--model_type=bert \
--model_name_or_path="./outputs/best_OCNLI_classify/" \
--output_filter_dir_name="./my_data/ogfilter_output/test_contradiction.json" \
--output_dir_add="test_contradiction/" \
--now_right_label=0 \
--output_dir="./my_data/ogfilter_output/" \
--task_name="ocnli" \
--do_predict \
...
def predict(args, model, tokenizer, label_list, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        logger.info("  sentences_num = %d", len(pred_dataset)/args.pred_batch_size)
        ...
        for step, batch in enumerate(pred_dataloader):
            ...
            if preds is None:
                ...
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)  # [21,3]
            pbar(step)
        print('\n')
        predict_label = np.argmax(preds, axis=1)

        output_file_dir=os.path.join(pred_output_dir,prefix,args.output_dir_add)
        if not os.path.exists(output_file_dir):
            os.makedirs(output_file_dir)
        output_submit_file = os.path.join(output_file_dir, "test_prediction.json")
        output_logits_file = os.path.join(output_file_dir, "test_logits")
        with open(output_submit_file, "w") as writer:
            for i, pred in enumerate(predict_label):
                json_d = {}
                json_d['id'] = i
                json_d['label'] = str(label_map[pred])
                json_d['label_probability'] = str(max(preds[i]))
                writer.write(json.dumps(json_d) + '\n')
        save_numpy(file_path=output_logits_file, data=preds)

        right_label_max_probability_id = [-1 for y in range(len(pred_dataloader))]
        all_wrong_sentences_list=[]
        for i in range(len(pred_dataloader)):
            for j in range(args.per_gpu_eval_batch_size):
                now_index = i * args.per_gpu_eval_batch_size + j
                if predict_label[now_index] == args.now_right_label:
                    if right_label_max_probability_id[i] == -1:
                        right_label_max_probability_id[i] = now_index
                    elif max(preds[right_label_max_probability_id[i]]) < max(preds[now_index]):
                        right_label_max_probability_id[i] = now_index
                    else:
                        continue
            if right_label_max_probability_id[i] == -1:
                right_label_max_probability_id[i] = i*args.per_gpu_eval_batch_size
                all_wrong_sentences_list.append(i)
        print("right_label_max_probability_id___len=", len(right_label_max_probability_id))
        print("all_wrong_sentences_index_list___len=", len(all_wrong_sentences_list))
        print(all_wrong_sentences_list)

        input_file = open("./my_data/test.json", "r", encoding='utf-8')
        output_file = open(args.output_filter_dir_name, "w", encoding='utf-8')
        print("Reading from {}, writing to {} ".format(input_file.name, output_file.name))
        index = 0
        i = 0
        for line in input_file.readlines():
            if i == right_label_max_probability_id[index]:
                output_file.write(line)
                index += 1
                if index==len(right_label_max_probability_id):
                    break
            i += 1
        assert index == len(right_label_max_probability_id)
        output_file.close()
        print("writing to {} over".format(output_file.name))
...
def main():
    ...
    parser.add_argument("--output_filter_dir_name", default=None, type=str, required=True,
                        help="The output directory where the model predictions will be written.")
    parser.add_argument("--now_right_label", default=0, type=int,
                        help="0: contradiction; 1: entailment; 2: neutral")
    parser.add_argument("--output_dir_add", default=None, type=str,
                        help="the add dir")
