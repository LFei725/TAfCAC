...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.max_steps > 0:  # default=-1
        t_total = args.max_steps  # Total optimization steps
        args.num_train_epochs = args.max_steps // (len(train_dataloader) // args.gradient_accumulation_steps) + 1
    else:  # t_total：Total optimization steps
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        pbar = ProgressBar(n_total=len(train_dataloader), desc='Training-'+str(_))
        for step, batch in enumerate(train_dataloader):
            ...
            outputs = model(**inputs)  # outputs[0]=loss outputs[1]=[probability]
        ...
        print(" ")
        results = evaluate1(args, model, tokenizer)
        for key in sorted(results.keys()):
            logger.info(" dev: %s = %s", key, str(results[key]))
        endtime = datetime.datetime.now()
        print("当前epoch-{}用时: ".format(_) + str((endtime - starttime).seconds) + "s")
...
def evaluate1(args, model, tokenizer):
    eval_task_names = (args.task_name,)
    eval_outputs_dirs = (args.output_dir,)
    results = {}
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, data_type='train')  # 加载&缓存
