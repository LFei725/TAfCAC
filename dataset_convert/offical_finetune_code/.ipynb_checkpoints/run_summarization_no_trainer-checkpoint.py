...
"""
--model_name_or_path /mnt/2-7new-try/task_augment/pretrain_models/mt5-small
--train_file ./data/neutral_train.10k.json
...
def parse_args():
    ...
    parser.add_argument(
        "--source_prefix",  # 在每个源文本之前添加前缀  此处应为  --source_prefix 'summarize: '
        type=str,
        default=None,
...
def main():
    ...
    if accelerator.is_local_main_process:
        datasets.utils.logging.set_verbosity_warning()   # 进到这里
    ...
    if accelerator.is_main_process:  # true
        ...
        elif args.output_dir is not None:  # true 搞一个输出文件夹
    ...
    def preprocess_function(examples): # remove pairs where at least one record is None  # 丢弃不全的pair
    ...
    with accelerator.main_process_first():   # 在preprocess_function中对每个batch进行编码
    ...
    for index in random.sample(range(len(train_dataset)), 1):  # 随便打印一个例子
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if args.with_tracking:
            total_loss = 0
        for step, batch in enumerate(train_dataloader):  # 12G不够
