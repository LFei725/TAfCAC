...
device = "cuda:0"
"""train:contradiction: 22:19:22"""  # time * 9
...
parser.add_argument('--out_path', type=str, default='../generate_data/overgenerate/', help='output directory')
    ...
    for nli_str in NLI_list:  # 三种task_prefix
        ...
        for line in input_file.readlines():  # 读doc
            ...
            for sentence in sentences:
                result = []
                result_index = 0
                input_ids = tokenizer(task_prefix + sentence, return_tensors="pt").input_ids.to(device)
                ...
                result.append(''.join(
                    tokenizer.decode(greedy_output.cpu().numpy()[0], skip_special_tokens=True)).replace(' ', ''))
                result_index += 1
                ...
                beam_output10 = model.generate(input_ids,
                                               ...
                                               eos_token_id=tokenizer.sep_token_id,
                                               num_beams=10,  # 保持10个束
                                               num_return_sequences=3,  # 返回3个最可能的序列
                ...
                for i, beam_output in enumerate(beam_output10):
                    ...
                    result_index += 1
                seed_list = [-156453, 85324, 345364, -782164, 3652, 632]
                for seed in seed_list:
                    ...
                    for i, sample_output1 in enumerate(sample_output):
                        result.append(''.join(
                            tokenizer.decode(sample_output1, skip_special_tokens=True)).replace(' ', ''))
                        result_index += 1
                    ...
                    for i, sample_output_top_p1 in enumerate(sample_output_top_p):
                        ...
                        result_index += 1
                assert result_index == 100  # 确定每个句子生成了100个

                for i in range(100):
                    nli_item = {}
                    nli_item["label"] = task_prefix[:-2]
                    nli_item["sentence1"] = sentences
                    nli_item["sentence2"] = result[i]
